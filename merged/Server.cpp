  ...
  public:  
    ...
    void thdread(){
      ...
      while(flag) { //while(conn)
        somen = recvfrom(socketid,recvmsg,sizeof(struct packet),0,(struct sockaddr *)&clientAddr, &addr_size) ; 
        ...
        if (recvmsg->options == ack){         
          ...
          sendto(socketid,recvmsg,sizeof(struct packet),0,(struct sockaddr *)&clientAddr,addr_size);
    ...
    void thdsend(){
       ...
       for(iter = 0; iter<wndw; iter++)
          msgs[iter] = new packet;//(struct packet *)malloc(sizeof(struct packet));
       ...
       while(1){
        ...
        //cout<<"enter:";        
           ...
           sendto(socketid,msgs[packno% wndw],sizeof(struct packet),0,(struct sockaddr *)&clientAddr,addr_size);
        ...
        while(resend){
          for(iter=0; iter<wndw; iter++){
             sendto(socketid,msgs[iter],sizeof(struct packet),0,(struct sockaddr *)&clientAddr,addr_size);
